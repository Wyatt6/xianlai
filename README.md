...
- **xianlai-core 核心程序 v1.0.0**：提供核心程序，包括公共注解、公共异常类、AOP 处理程序、日志打印程序、SaToken 配置、Feign 配置、Feign 消费者服务声明、响应对象封装类、工具类；
- **xianlai-infra-redis Redis 访问封装程序 v1.0.0**：提供对 Redis 连接的统一配置；
- **xianlai-infra-mysql MySQL 访问封装程序 v1.0.0**：提供对 MySQL 连接和 Druid 工具的统一配置。
