...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("select distinct new fun.xianlai.app.iam.model.entity.other.UserInfo(u.id, u.username, u.registerTime, u.active, u.isDelete, " +
            ...
            "      and (?7 is null or pf.name like %?7%) " +
            "      and (?8 is null or pf.gender = ?8) " +
            "      and (?9 is null or pf.employeeNo like %?9%) " +
            "      and (?10 is null or pf.phone like %?10%) " +
            "      and (?11 is null or pf.email like %?11%) " +
            "      and (?12 is null or r.identifier like %?12%) " +
            "      and (?13 is null or p.identifier like %?13%)")
