...
import fun.xianlai.system.model.entity.SysOption;
import fun.xianlai.system.repository.SysOptionRepository;
...
public class OptionServiceImpl implements OptionService {
    ...
    private static final String BACK_OPTION_CACHE_PREFIX = "backOption:";
    ...
    public void cacheBackLoadOptions() {
        ...
        if (options != null) {
            for (SysOption option : options) {
                redis.opsForValue().set(BACK_OPTION_CACHE_PREFIX + option.getOptionKey(), option.getOptionValue(), Duration.ofHours(CACHE_HOURS));
            }
        }
    }
    ...
    public void cacheCertainBackLoadOption(String key) {
        ...
        redis.delete(BACK_OPTION_CACHE_PREFIX + key);
        if (option.isPresent()) {
            redis.opsForValue().set(BACK_OPTION_CACHE_PREFIX + key, option.get().getOptionValue(), Duration.ofHours(CACHE_HOURS));
        }
    }
    ...
    public String getCertainBackLoadOptionValueFromCache(String key) {
        String value = (String) redis.opsForValue().get(BACK_OPTION_CACHE_PREFIX + key);
        if (value == null) {
            self.cacheCertainBackLoadOption(key);
            value = (String) redis.opsForValue().get(BACK_OPTION_CACHE_PREFIX + key);
        }
        return value;
    }
