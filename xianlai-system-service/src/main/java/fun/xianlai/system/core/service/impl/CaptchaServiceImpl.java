...
import fun.xianlai.common.utils.file.ImageUtils;
...
//        kaptcha.image.height  验证码图片高度  默认为50
    ...
    public Map<String, String> generateCaptcha() {
        try {
            ...
            Integer length = configService.getConfigOrDefault(tenantId, "captcha.length", Integer.class, 5);
            Integer expireSeconds = configService.getConfigOrDefault(tenantId, "captcha.expireSeconds", Integer.class, 60);
            ...
            String captchaImageBase64 = ImageUtils.bufferedImageToBase64(captchaImage, "jpeg");
            ...
            result.put("captchaImage", captchaImageBase64);
            return result;
        } catch (IOException e) {
            throw new BizException(RetCode.CAPTCHA_GENERATE_FAIL, "验证码生成失败");
        }
    }
    ...
    public void verifyCaptcha(String captchaKey, String captcha) {
        ...
        if (captchaText == null) {
            throw new BizException(RetCode.CAPTCHA_EXPIRED, "验证码已失效");
        } else {
            if (captcha.equalsIgnoreCase(String.valueOf(captchaText))) {
                redis.delete("captcha:" + captchaKey);
            } else {
                throw new BizException(RetCode.CAPTCHA_INVALID, "验证码错误");
            }
        }
    }
}
